...
int parse(fname, n_ad, m_ad, nodes_ad, arcs_ad, cap_ad, node_min_ad )
...
long    **cap_ad;              /* address of the array of capacities */
...
while (fgets(in_line, MAXLINE, file) != NULL )
  {
  no_lines ++;
  ...
  switch (in_line[0])
    {
      ...
      case 'p':                  /* problem description      */
		...
		m = 2*m; /* acount for anti-parallel edges (jc) */
		node_max = 0;
		node_min = n;
        /* allocating memory for  'nodes', 'arcs'  and internal arrays */
                nodes    = (node*) calloc ( n+4, sizeof(node) );
		arcs     = (arc*)  calloc (2*m+2, sizeof(arc) );
	        arc_tail = (long*) calloc (2*m,   sizeof(long) ); 
		arc_first= (long*) calloc ( n+4, sizeof(long) );
      ...
      case 'a':                    /* arc description */
		...
		// undirected version - add anti-parallel edges
		for (i_edges=0; i_edges < 2; ++i_edges)
		  {
		    ...
		    ( arc_current + 1 ) -> resCap    = 0; // directed graph (jc)
...
if ( no_alines < m/2 ) /* not enough arcs */
...
for ( i = node_min + 1; i <= node_max + 1; i ++ ) 
  {
    (nodes + i-1)->degree = arc_first[i]/2;
